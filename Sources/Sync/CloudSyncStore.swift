...
final class CloudSyncStore {
    ...
    func markEverythingDirtyAndClearServerMetadata() async throws {
        try await db.write { db in
            try db.execute(sql: "UPDATE book SET needsSync = 1")
        ...
        try await db.read { db in
            ...
            if remaining > 0 {
                ...
                for tombstone in tombstones {
                    guard let type = CloudSyncRecordType(rawValue: tombstone.recordType) else { continue }
                    result.append(PendingCloudChange(recordType: type, syncID: tombstone.syncID, kind: .delete))
                }
    ...
    /// Applies fetched records with books first so annotation foreign keys are
    /// present even when CloudKit delivers a batch in a different order.
    func applyFetchedRecords(_ records: [CKRecord]) async throws -> [CKRecord.ID] {
        let ordered = records.sorted { lhs, rhs in
            rank(lhs.recordType) < rank(rhs.recordType)
        }
    ...
    /// Returns true when a newer local value won and should be sent back.
    ...
    private func applyBook(_ record: CKRecord) async throws -> Bool {
        ...
        if let existing, !CloudSyncMergePolicy.remoteWins(localUpdatedAt: existing.updatedAt, remoteUpdatedAt: remoteUpdatedAt) {
            if !existing.needsSync {
                try await markDirty(type: .book, syncID: existing.syncID)
            }
            return true
        }
        ...
        {
            let preferredName = (record["fileName"] as? String) ?? UUID().uuidString
            let destination = Paths.documents.appendingUniquePathComponent(safeFilename(preferredName))
            try FileManager.default.copyItem(at: assetURL, to: destination.url)
            let anyURL: AnyURL = Paths.documents.relativize(destination)?.anyURL ?? destination.anyURL
            resolvedURL = anyURL.string
        ...
        if !hasReachableCover(existing),
           let coverAsset = record["cover"] as? CKAsset,
           let assetURL = coverAsset.fileURL
        {
            let preferredName = (record["coverName"] as? String) ?? UUID().uuidString
            let destination = Paths.covers.appendingUniquePathComponent(safeFilename(preferredName))
            try FileManager.default.copyItem(at: assetURL, to: destination.url)
        ...
        let progression = (record["progression"] as? NSNumber)?.doubleValue ?? locator?.locations.totalProgression ?? 0
        let preferencesJSON = record["preferencesJSON"] as? String
        let authors = record["authors"] as? String

        do {
            try await db.write { db in
                if var local = try Book.filter(Book.Columns.syncID == syncID).fetchOne(db)
                    ?? (identifier.flatMap { try? Book.filter(Book.Columns.identifier == $0).fetchOne(db) })
                {
                    ...
                    if local.syncID != syncID, !local.syncID.isEmpty {
                        try SyncTombstone(
                    ...
                    local.preferencesJSON = preferencesJSON
                    local.syncID = syncID
    ...
    private func applyBookmark(_ record: CKRecord) async throws -> Bool {
        ...
        return try await db.write { db in
            ...
            if var local = try Bookmark.filter(Bookmark.Columns.syncID == syncID).fetchOne(db) {
                ...
                local.bookId = bookID
                local.locator = locator
                local.progression = (record["progression"] as? NSNumber)?.doubleValue ?? locator.locations.totalProgression
                ...
                bookmark.progression = (record["progression"] as? NSNumber)?.doubleValue ?? locator.locations.totalProgression
    ...
    private func applyHighlight(_ record: CKRecord) async throws -> Bool {
        ...
        return try await db.write { db in
            ...
            if var local = try Highlight.filter(Highlight.Columns.syncID == syncID).fetchOne(db) {
                ...
                highlight.progression = (record["progression"] as? NSNumber)?.doubleValue ?? locator.locations.totalProgression
    ...
    func applyRemoteDeletion(_ recordID: CKRecord.ID) async throws {
        ...
        try await db.write { db in
            ...
            case .book:
                if let book = try Book.filter(Book.Columns.syncID == syncID).fetchOne(db) {
                    if let file = try? book.absoluteFileURL(), let file {
        ...
        for file in filesToDelete where Paths.isAppFile(at: file.fileURL!) {
    ...
    private func markDirty(type: CloudSyncRecordType, syncID: String) async throws {
        try await db.write { db in
            switch type {
            case .book:
                try Book.filter(Book.Columns.syncID == syncID)
                    .updateAll(db, Book.Columns.needsSync.set(to: true))
            case .bookmark:
                try Bookmark.filter(Bookmark.Columns.syncID == syncID)
                    .updateAll(db, Bookmark.Columns.needsSync.set(to: true))
            case .highlight:
                try Highlight.filter(Highlight.Columns.syncID == syncID)
