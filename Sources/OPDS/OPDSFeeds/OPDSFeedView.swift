...
struct OPDSFeedView: View {
    ...
    var body: some View {
        mainContent
            .navigationTitle(viewModel.feed?.metadata.title ?? NSLocalizedString("opds_loading", comment: ""))
        ...
        ScrollView {
            LazyVStack(spacing: 0) {
                if viewModel.feed != nil {
                    ...
                    Text(NSLocalizedString("opds_load_error", comment: ""))
                        .padding()
                } else {
                    ProgressView()
                        .padding()
    ...
    private func buildNoneView() -> some View {
        if let error = viewModel.error {
            Text(String(format: NSLocalizedString("opds_load_error_detail", comment: ""), error.localizedDescription))
        } else {
            Text(NSLocalizedString("opds_no_content", comment: ""))
        }
    }
