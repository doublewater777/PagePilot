...
- `OnboardingView`: `watchIntroScreen` displays hero, title, subtitle, 3 value props (Watch remote, multi-format library with Wi-Fi, iCloud private sync), and Continue CTA that advances to `publicationScreen`. Book selection in `publicationScreen` directly opens the Reader via `persistAndOpenReader()`.
